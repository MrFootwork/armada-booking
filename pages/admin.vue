<script setup lang="ts">
	...
	// BUG🐞 in Production: can't navigate to this page
	// => set admin access and enable admin pages
	// => error handle this 500 error
...
<template>
	<div>
		...
		<button @click="addDayWithSample(daySelected)">
			days.post with sample
		</button>
		<button @click="deleteDays('all')">days.delete.all</button>
		<button @click="resetDays()">days.reset</button>
		<button
			@click="addSlotInStore(daySelected)"
		...
		<br /><span>JS Date: {{ daySelected }}</span> <br /><span
			>ISO: {{ daySelected.toISOString() }}</span
		>
		<br /><span>String: {{ daySelected.toString() }}</span> <br /><span
			>UTC: {{ daySelected.toUTCString() }}</span
		>
		<br /><span
			>useDate().resetTime(): {{ useDate(daySelected).resetTime() }}</span
		>

		<div class="days-wrapper">
			<div class="day-item" v-for="day in days">
				{{
